...
function autocontrast(pixData, cutoff, invert) {
  ...
  for (let x = 0; x < width; x++) {
    pixelCache[x] = [];
    for (let y = 0; y < height; y++) {
      let pixelValue = Math.min(255, Math.max(0, (luma(x, y) - low) * scale));
      if (invert) {
        pixelValue = 255 - pixelValue;
      }
      pixelCache[x][y] = pixelValue;
    }
  }
  return (x, y) => {
...
function postLinesJson(data){
  ...
  // Send the JSON string to the main thread
  postMessage(['json-data', jsonString])
}

function postLinesOriginal(data){
